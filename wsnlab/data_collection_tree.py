...
class SensorNode(wsn.Node):
    ...
    # Network reorganization: disconnect all children and make them rejoin
        ...
        # Also record for Figure 7/11 statistics
        if self.id not in NODE_DEATHS:
            NODE_DEATHS[self.id] = {
                "time": self.now,
                "role": str(self.role) # Store role at time of death
            }
    ...
    def send_ch_nomination(self):
        ...
        for gui, neigh in self.neighbors_table.items():
            src = neigh.get('source')   # Addr
            # Skip if source is invalid or in blacklist
            if src is None or not hasattr(src, 'net_addr'):
                continue
    ...
    def route_and_forward_package(self, pck):
        ...
        # Safely get dest object
        dest_obj = pck.get('dest')
        
        if self.ch_addr is not None and dest_obj is not None and hasattr(dest_obj, 'net_addr'):
            if dest_obj.net_addr == self.ch_addr.net_addr:
                pck['next_hop'] = dest_obj
                path_type = "TREE"
            else:
                for child_gui, child_networks in self.child_networks_table.items():
                    if dest_obj.net_addr in child_networks:
                        pck['next_hop'] = self.neighbors_table[child_gui]['addr']
                        path_type = "TREE"
                        break
        elif self.role == Roles.ROUTER and dest_obj is not None and hasattr(dest_obj, 'net_addr'):
            for child_gui, child_networks in self.child_networks_table.items():
                if dest_obj.net_addr in child_networks:
                    pck['next_hop'] = self.neighbors_table[child_gui]['addr']
                    path_type = "TREE"
        ...
        if self.role in (Roles.ROOT, Roles.CLUSTER_HEAD):
            ...
            if self.role == Roles.CLUSTER_HEAD and pck['type'] == 'CH_NOMINATION_ACK':
                if getattr(self, 'awaiting_ack', False) and getattr(self, 'ch_nominee', None):
                    ...
                    if (source is not None and hasattr(source, 'net_addr') and
                            source.net_addr == self.ch_nominee[0] and
                            source.node_addr == self.ch_nominee[1]):
    ...
    def on_timer_fired(self, name, *args, **kwargs):
        ...
        if name == 'TIMER_ARRIVAL':
            ...
            # Logic Fix: Only share tables if Mesh is enabled
            if getattr(config, 'ENABLE_MESH', True):
...
def run_simulation(node_count=None, visual=True, duration=5000):
    global sim
...
def log_connectivity(sim_ref):
    ...
    global NETWORK_LIFETIME, NETWORK_FORMED
    
    # Idle Power Drain Simulation (Baseline Energy Consumption)
    IDLE_POWER_W = 0.0006 
    dt = config.POWER_SAMPLING_INTERVAL
    idle_drain = IDLE_POWER_W * dt
    
    for n in ALL_NODES:
        if not getattr(n, "failed", False) and n.id != ROOT_ID:
             n.power -= idle_drain
             if n.power <= MIN_ENERGY_J:
                 n._die_of_energy()

    total = len(ALL_NODES)
    alive_nodes = [n for n in ALL_NODES if getattr(n, "power", 0) > MIN_ENERGY_J]
    # AVG over TOTAL nodes, not just alive ones (Fix Survivor Bias)
    # Dead nodes contribute ~MIN_ENERGY_J (essentially 0 relative to E0)
    avg_power = sum(n.power for n in ALL_NODES) / total if total > 0 else 0
    ...
    total = len(ALL_NODES)
    connected_count = sum(1 for n in ALL_NODES 
                    if getattr(n, "role", None) in {Roles.REGISTERED, Roles.CLUSTER_HEAD, Roles.ROOT, Roles.ROUTER}
                    and getattr(n, "power", 0) > MIN_ENERGY_J)
    
    connectivity = connected_count / total if total > 0 else 0.0
    
    # Capture Packet Stats
    tx = getattr(sim_ref, "total_tx_attempts", 0)
    dropped = getattr(sim_ref, "total_tx_dropped", 0)
    ...
    # === NEW LOGIC from User Request ===
    # Relaxed Thresholds: 50% Formation to handle congestion
    THRESH = 0.50
    
    # Track if network ever formed (reached 50% connectivity)
    if not NETWORK_FORMED and connectivity >= THRESH:
        NETWORK_FORMED = True
        print(f"✅ NETWORK FORMED at t={sim_ref.now:.2f} (connectivity={connectivity:.2%})")
    
    # Only record lifetime AFTER network formed AND connectivity drops below Threshold
    if NETWORK_FORMED and connectivity < THRESH and NETWORK_LIFETIME is None:
        NETWORK_LIFETIME = sim_ref.now
        print(f"📉 NETWORK LIFETIME REACHED at t={sim_ref.now:.2f} (connectivity={connectivity:.2%})")
...
# Global metrics containers for advanced plots (Figs 6-11)
METRICS_OVER_TIME = []  # List of dicts: {time, avg_energy, connectivity, tx, dropped}
NODE_DEATHS = {}        # Dict: {node_id: {time, role}}
NETWORK_FORMED = False  # Track if network ever reached steady state
...
def run_simulation(node_count=None, visual=True, duration=5000, nodes_to_kill=1):
    ...
    global NETWORK_LIFETIME, NETWORK_FORMED
    global ROOT_ID
    global METRICS_OVER_TIME, NODE_DEATHS
    ...
    # 7. Metrics Over Time (list of dicts)
    # 8. Node Deaths (dict)
    return avg_join_time, MAX_ORPHAN_COUNT, final_orphan_count, NETWORK_LIFETIME, attempts, dropped, METRICS_OVER_TIME, NODE_DEATHS

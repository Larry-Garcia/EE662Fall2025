...
SIM_NODE_PLACING_CELL_SIZE =60 #60 okay with performance #50 is good with power  #80 works but overlap with ower # cell size to place one node
SIM_DURATION = 5000  # simulation Duration in seconds
SIM_TIME_SCALE = 0  # 0 for max speed (headless/fast-forward), >0 for real-time factor
...
ENABLE_MESH = True  # set True to keep mesh routing/heartbeat export, False to skip for speed
...
POWER_SAMPLING_INTERVAL = 50  # interval to sample average node power (sim seconds)

# routing properties
NEIGHBOR_TABLE_MAX_HOPS = 2  # max hops for neighbor table sharing
MAX_MESH_ROUTE_HOPS = 4  # max hops to attempt mesh routing
...
# Energy thresholds
INITIAL_ENERGY_J = 2.0  # starting energy per node (pick value that lets nodes actually die during your sim)
...
NETWORK_DEATH_THRESHOLD = 0.50 # 0.05 is good to see amount of death nodes # 50% of nodes dead
